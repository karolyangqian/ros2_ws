...
from math import atan2, sin, cos, pi, fabs, sqrt
import random
import time
...
class OdometryMotionModel(Node):
    ...
    def __init__(self):
        ...
        if self.nr_samples >= 0:
            ...
            self.get_logger().error('Number of samples must be greater than 0\nNumber of samples requested: %d', self.nr_samples)
        ...
        self.get_logger().info('Odometry Motion Model node has been started')
    ...
    def odomCallback(self, odom):
        ...
        delta_trasl = sqrt(odom_x_increment**2 + odom_y_increment**2)
        delta_rot2 = angle_diff(odom_theta_increment, delta_rot1)
        
        rot1_variance = self.alpha1 * fabs(delta_rot1) + self.alpha2 * delta_trasl
        trasl_variance = self.alpha3 * delta_trasl + self.alpha4 * (fabs(delta_rot1) + fabs(delta_rot2))
        rot2_variance = self.alpha1 * fabs(delta_rot2) + self.alpha2 * delta_trasl
        ...
        self.get_logger().info('Samples published')


def main():
    rclpy.init()
    odom_motion_model = OdometryMotionModel()
    rclpy.spin(odom_motion_model)
    rclpy.destroy_node(odom_motion_model)
    rclpy.shutdown()
